  ...
  heaping(index) {
    if (this.leftChild(index) !== undefined) {
      if (this.rightChild(index) !== undefined) {
        if (
          this.teams[index].teamscore > this.leftChild(index).teamscore ||
          this.teams[index].teamscore > this.rightChild(index).teamscore
        ) {
          let smallerIndex =
            this.leftChild(index).teamscore < this.rightChild(index).teamscore
        ...
        if (this.teams[index].teamscore > this.leftChild(index).teamscore) {
...
let sortStudents = Students.data.sort((a, b) => a.score - b.score); //배열 정렬

//만들려고 하는 팀 수보다 학생수가 적으면 팀을 만들수 없어서
//에러 처리를 해주었습니다. 콘솔이 아니라 팝업창을 띄운다던가 하면 더 좋을듯합니다
let teamCount = Students.TeamCount;
...
for (let i = 0; i < teamCount; i++) {
  ...
  TeamList.teamlist.unshift(team.team);
